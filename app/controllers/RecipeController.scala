  ...
  def addSubmit() = SecuredAction(authorize = WithRole(RoleEnums.ADMIN))(parse.multipartFormData) { implicit request =>
    ...
    contentForm.bindFromRequest.fold(
      ...
      contentData => {
        ...
        Redirect(controllers.routes.RecipeController.editIndex()).flashing(FlashMsgConstants.Success -> successMessage)
      }
    )
  ...
  def delete(objectId: UUID) = SecuredAction(authorize = WithRole(RoleEnums.ADMIN)) { implicit request =>
    ...
    result match {
      ...
      case false =>
        ...
        Redirect(controllers.routes.RecipeController.editIndex()).flashing(FlashMsgConstants.Error -> errorMessage)
    }

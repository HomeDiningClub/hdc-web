...
trait EventRepository extends GraphRepository[Event] {
  ...
  @Query("MATCH (up:`UserProfile`{objectId:{userProfileObjectId}})-[:HOSTS_EVENTS]-(e:Event) RETURN COUNT(e)")
  def getCountOfMyEvents(@Param("userProfileObjectId") userProfileObjectId: String): Int

  @Query("MATCH (uc {objectId:{0}})-[:IN_PROFILE]->(up:UserProfile)-[:HOSTS_EVENTS]-(e:Event) OPTIONAL MATCH (c:`County`)<-[:`LOCATION_AT`]-(up) OPTIONAL MATCH (up)-[:`MAIN_IMAGE`]-(userImage:`ContentFile`) optional match (e)-[:IMAGES]-(eventImages:`ContentFile`) optional match (e)-[g]-(ux:UserCredential) optional match (e)-[:`MAIN_IMAGE`]-(mainImage:`ContentFile`) OPTIONAL MATCH (e)-[:`EVENT_TIMES`]-(ed:`EventDate`) WHERE toFloat(ed.eventDateTime) > timestamp() return e.name as EventName, e.preAmble as EventPreAmble, e.mainBody as EventMainBody, e.objectId as EventObjectId, COLLECT(eventImages.storeId) as EventImages, COLLECT(mainImage.storeId) as MainImage, COLLECT(userImage.storeId) as UserImage, e.price as EventPrice, up.profileLinkName as ProfileLinkName, e.eventLinkName as EventLinkName, c.name as CountyName, COLLECT(ed.eventDateTime) as EventDateTimes")
  ...
  @Query(
    ...
    countQuery  = "MATCH (uc {objectId:{0}})-[:IN_PROFILE]->(up:UserProfile)-[:HOSTS_EVENTS]-(e:Event) OPTIONAL MATCH (c:`County`)<-[:`LOCATION_AT`]-(up) OPTIONAL MATCH (up)-[:`MAIN_IMAGE`]-(userImage:`ContentFile`) optional match (e)-[:IMAGES]-(eventImages:`ContentFile`) optional match (e)-[g]-(ux:UserCredential) optional match (e)-[:`MAIN_IMAGE`]-(mainImage:`ContentFile`) RETURN Count(e.objectId)")
  ...
  @Query(
    ...
    countQuery  = "MATCH (up:`UserProfile`)-[:`HOSTS_EVENTS`]-(e:`Event`) WHERE {role} IN up.role OPTIONAL MATCH (c:`County`)<-[:`LOCATION_AT`]-(up) OPTIONAL MATCH (up)-[:`AVATAR_IMAGE`]-(userImage:`ContentFile`) OPTIONAL MATCH (e)-[:`MAIN_IMAGE`]-(mainImage:`ContentFile`) RETURN Count(e.objectId)")
  def findPopularEvents(@Param("role") role: String, pageable: Pageable) : Page[EventData]

  // Fetch events that has TagWord defined
  @Query("MATCH (tw:`TagWord`{objectId: {tagWordId} })<-[:`TAGGED_ON`]-(up:`UserProfile`)-[:`HOSTS_EVENTS`]-(e:`Event`) WHERE {role} IN up.role WITH up,tw,e OPTIONAL MATCH (c:`County`)<-[:`LOCATION_AT`]-(up) OPTIONAL MATCH (up)-[:`AVATAR_IMAGE`]-(userImage:`ContentFile`) OPTIONAL MATCH (e)-[:`MAIN_IMAGE`]-(mainImage:`ContentFile`) OPTIONAL MATCH (e)-[:`EVENT_TIMES`]-(ed:`EventDate`) WHERE toFloat(ed.eventDateTime) > timestamp() WHERE tw.objectId = {tagWordId} RETURN e.name as EventName, e.preAmble as EventPreAmble, e.mainBody as EventMainBody, e.objectId as EventObjectId, COLLECT(mainImage.storeId) as MainImage, COLLECT(userImage.storeId) as UserImage, e.price as EventPrice, up.profileLinkName as ProfileLinkName, e.eventLinkName as EventLinkName, tw.name as TagWord, c.name as CountyName, COLLECT(ed.eventDateTime) as EventDateTimes")
  ...
  @Query(
    ...
    countQuery  = "MATCH (tw:`TagWord`{objectId: {tagWordId} })<-[:`TAGGED_ON`]-(up:`UserProfile`)-[:`HOSTS_EVENTS`]-(e:`Event`) WHERE {role} IN up.role WITH up,tw,e OPTIONAL MATCH (c:`County`)<-[:`LOCATION_AT`]-(up) OPTIONAL MATCH (up)-[:`AVATAR_IMAGE`]-(userImage:`ContentFile`) OPTIONAL MATCH (e)-[:`MAIN_IMAGE`]-(mainImage:`ContentFile`) WHERE tw.objectId = {tagWordId} RETURN Count(e.objectId)")
  ...
  @Query(
    value       = "MATCH (c:`County`{objectId: {countyId} })<-[:`LOCATION_AT`]-(up:`UserProfile`)-[:`HOSTS_EVENTS`]-(e:`Event`) WHERE {role} IN up.role WITH up,c,e OPTIONAL MATCH (up)-[:`AVATAR_IMAGE`]-(userImage:`ContentFile`) OPTIONAL MATCH (e)-[:`MAIN_IMAGE`]-(mainImage:`ContentFile`) OPTIONAL MATCH (e)-[:`EVENT_TIMES`]-(ed:`EventDate`) WHERE toFloat(ed.eventDateTime) > timestamp() RETURN e.name as EventName, e.preAmble as EventPreAmble, e.mainBody as EventMainBody, e.objectId as EventObjectId, COLLECT(mainImage.storeId) as MainImage, COLLECT(userImage.storeId) as UserImage, e.price as EventPrice, up.profileLinkName as ProfileLinkName, e.eventLinkName as EventLinkName, c.name as CountyName, COLLECT(ed.eventDateTime) as EventDateTimes",
    countQuery  = "MATCH (c:`County`{objectId: {countyId} })<-[:`LOCATION_AT`]-(up:`UserProfile`)-[:`HOSTS_EVENTS`]-(e:`Event`) WHERE {role} IN up.role WITH up,c,e OPTIONAL MATCH (up)-[:`AVATAR_IMAGE`]-(userImage:`ContentFile`) OPTIONAL MATCH (e)-[:`MAIN_IMAGE`]-(mainImage:`ContentFile`) RETURN Count(e.objectId)")
  ...
  @Query(
    ...
    countQuery  = "MATCH (tw:`TagWord`{objectId: {tagWordId} })<-[:`TAGGED_ON`]-(up:`UserProfile`)-[:`HOSTS_EVENTS`]-(e:`Event`), (c:`County`{objectId: {countyId} })<-[:`LOCATION_AT`]-(up) WHERE {role} IN up.role WITH up,c,tw,e OPTIONAL MATCH (up)-[:`AVATAR_IMAGE`]-(userImage:`ContentFile`) OPTIONAL MATCH (e)-[:`MAIN_IMAGE`]-(mainImage:`ContentFile`) RETURN Count(e.objectId)")
